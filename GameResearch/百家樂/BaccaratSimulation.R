...
while(exp.index < target.turn){
  ...
  if(length(pool.0) < 70){
    ...
    cat('Current Turn : ' , exp.index , ' on-going ...\n')
  }else{
    cat('Current Turn : ' , exp.index , ' on-going ...\n')
  }
  ...
  if(any(c(Player.Points,Banker.Points) >= 8) ){
    ...
    # 計算龍七發生時，人工指標(特定牌卡濃度) 
    # 這裡還沒發生，因為沒有到第三張牌
    save.vt = c(Player.Card.vt , Banker.Card.vt , Player.Points , Banker.Points , Game.Result , length(pool.1) , exp.index , Highlight.index)
    ...
    if(Player.Points <6){
      ...
      # 不到6點 => 需要補牌
      sp.1 = sample(1:length(pool.1) , size=1 , replace = F)
      # 更新牌池
      pool.2 = pool.1[-sp.1]
      # 分派牌卡
      Player.Card.vt[3] = pool.1[sp.1]
      # 更新閒家點數
      ...
      if(Banker.Points < 3){
        # 不到3點 => 需要補牌
        sp.2 = sample(1:length(pool.2) , size=1 , replace = F)
        # 更新牌池
        pool.3 = pool.2[-sp.2]
        # 分派牌卡
        Banker.Card.vt[3] = pool.2[sp.2]
        # 更新莊家點數
        ...
        # 儲存資料
        save.vt = c(Player.Card.vt , Banker.Card.vt , Player.Points , Banker.Points , Game.Result , length(pool.3) , exp.index , Highlight.index)
        Total.df = rbind(Total.df , save.vt)
        pool.0 <- pool.3
        ...
        if(sub('.','',last(Player.Card.vt))=='8'){
          ...
          # 無觸發莊家補牌，因此無須計算指標
          # 儲存資料
          save.vt = c(Player.Card.vt , Banker.Card.vt , Player.Points , Banker.Points , Game.Result , length(pool.2) , exp.index , Highlight.index)
          Total.df = rbind(Total.df , save.vt)
          pool.0 <- pool.2
        }else{
          # 閒家沒有補到8 => 需要補牌
